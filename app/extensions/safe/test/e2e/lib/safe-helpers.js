...
export const createRandomDomain = async ( content, path, service, authedApp ) => {
    ...
    return app.mutableData
        ...
        .then( serviceMdata => serviceMdata.quickSetup().then( () => {
            const nfs = serviceMdata.emulateAs( 'NFS' );
            // let's write the file
            return nfs
                .create( content )
                .then( file => nfs.insert( path || '/index.html', file ) )
                .then( () => app.crypto.sha3Hash( domain ).then( dnsName => app.mutableData
                    .newPublic( dnsName, TAG_TYPE_DNS )
                    .then( dnsData => serviceMdata.getNameAndTag().then( res => {
                        const payload = {};
